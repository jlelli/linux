...
static int tg_rt_schedulable(struct task_group *tg, void *data)
{
	...
	period = ktime_to_ns(tg->dl_bandwidth.dl_period);
	runtime = tg->dl_bandwidth.dl_runtime;

	if (tg == d->tg) {
		period = d->dl_period;
		runtime = d->dl_runtime;
	}
	...
	/*
	 * Ensure we don't starve existing DEADLINE tasks.
	 */
	if (!runtime && tg_has_dl_tasks(tg))
		return -EBUSY;
	...
	list_for_each_entry_rcu(child, &tg->children, siblings) {
		period = ktime_to_ns(child->dl_bandwidth.dl_period);
		...
		u64 rt_period, u64 rt_runtime)
{
	...
	tg->dl_bandwidth.dl_period = ns_to_ktime(rt_period);
...
int sched_group_set_rt_runtime(struct task_group *tg, long rt_runtime_us)
{
	...
	rt_period = ktime_to_ns(tg->dl_bandwidth.dl_period);
...
long sched_group_rt_period(struct task_group *tg)
{
	...
	rt_period_us = ktime_to_ns(tg->dl_bandwidth.dl_period);
...
{
	print_rt_rq(m, cpu, &cpu_rq(cpu)->rt);
}
